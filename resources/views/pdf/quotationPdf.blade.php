    ...
    <div class="page-container">
        <div class="header-box">
            ...
            <div class="document-info">
                ...
                <div class="document-number">No. {{ $quotation->quotation_number }}</div>
                <div class="document-date">Date: {{ $quotation->created_at->format('F j, Y') }}</div>
            </div>
        </div>
        ...
        <div class="billing-box">
            <table class="billing-table">
                <thead>
                    <tr>
                        <th style="width: 40%;">Quotation To</th>
                        <th style="width: 60%;">Quotation From</th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        <td style="width: 40%;">
                            @if ($quotation->clientDetail)
                                ...
                                <div><b>Address:</b> {{ $quotation->clientDetail->address ?? 'N/A' }}</div>
                                <div><b>Phone:</b> {{ $quotation->clientDetail->phone ?? 'N/A' }}</div>
                                <div><b>Email:</b> {{ $quotation->clientDetail->email ?? 'N/A' }}</div>
                            @elseif($quotation->leadDetail)
                                <div><strong>{{ $quotation->leadDetail->name }}</strong></div>
                                <div><b>Contact:</b> {{ $quotation->leadDetail->contact_person ?? 'N/A' }}</div>
                                <div><b>Phone:</b> {{ $quotation->leadDetail->phone ?? 'N/A' }}</div>
                                <div><b>Email:</b> {{ $quotation->leadDetail->email ?? 'N/A' }}</div>
                            @else
        ...
        <div class="totals-section" style="width: 50%; float: right; margin-top: 30px;">
            <table class="totals-table">
                <tr>
                    <td>Subtotal:</td>
                    <td style="text-align: right;">{{ number_format($quotation->sub_total, 2) }}</td>
                </tr>
            ...
            <div class="total-highlight">
                <strong>Total Amount: {{ number_format($quotation->total, 2) }}</strong>
            </div>
        </div>
        ...
        @if ($quotation->payment_terms)
            <div class="terms-section" style="margin-top: 40px;">
                ...
                <div class="terms-content">{!! nl2br($quotation->payment_terms) !!}</div>
            </div>
        @endif
        ...
        @if ($quotation->terms_conditions)
            <div class="terms-section">
                ...
                <div class="terms-content">{!! nl2br($quotation->terms_conditions) !!}</div>
            </div>
        @endif
