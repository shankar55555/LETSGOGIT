<template>
  <VMenu>
    <template v-slot:activator="{ props }">
      <VBtn v-bind="props" variant="outlined" color="primary" v-tooltip="'Actions'" size="small" icon="tabler-files" />
    </template>
    <VList class="box_shadow">
      <VListItem class="mx-0" v-if="$can('leads', 'export-list')">
        <VBtn prepend-icon="tabler-download" @click="exportLeads" variant="outlined" size="small" color="primary">
          Export
        </VBtn>
      </VListItem>
      <VListItem class="mx-0" v-if="$can('leads', 'create')">
        <VBtn prepend-icon="tabler-upload" @click="$refs.fileInput.click()" variant="outlined" size="small"
          color="primary">
          Upload</VBtn>
        <input ref="fileInput" type="file" accept=".xls,.xlsx" style="display: none;" @change="handleFileImport" />
      </VListItem>
      <VListItem class="mx-0" v-if="$can('leads', 'export-list')">
