...
const validateAmountNotExceedBalance = (value) => {
    if (!value) return 'Amount is required'
    if (parseFloat(value) > parseFloat(Payment.balance)) {
        return 'Amount cannot be greater than Invoice Balance'
    }
    return true
}

export const nonZeroPositionRule = value => {
    if (value === 0 || value === '0') {
        return true;
    }
    return value > 0 || 'Position must be greater than 0';
};
