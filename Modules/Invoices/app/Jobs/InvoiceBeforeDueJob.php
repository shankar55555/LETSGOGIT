...
class InvoiceBeforeDueJob implements ShouldQueue
{
    use Dispatchable, InteractsWithQueue, Queueable, SerializesModels;

    private const LOG_CHANNEL = 'invoice';
    private const LOG_PREFIX = 'Invoice: Days Before Due Notification';
    ...
    public function handle(): void
    {
        ...
        try {
            ...
            $invoiceQuery = Invoice::whereNot('status', InvoiceConst::PAID)
                ->where('due_date', '>', now()); // Only future due dates
            ...
            $invoiceQuery->where(function ($query) use ($conditions, $logic) {
                foreach ($conditions as $condition) {
                    ...
                    if ($datatype === 'date' && $slug === InvoiceConst::RULE_DAYS_BEFORE_DUE) {
    ...
    /**
     * Get the active rule for days before due notification.
     */
    private function getActiveRule(): ?Rule
    {
        $rule = Rule::where('rule_slug', InvoiceConst::RULE_DAYS_BEFORE_DUE)
    ...
    private function processInvoice(int $invoiceId): void
    {
        try {
            ...
            $notificationHelper->handle(InvoiceConst::RULE_DAYS_BEFORE_DUE, $data, null, loginUserId());
