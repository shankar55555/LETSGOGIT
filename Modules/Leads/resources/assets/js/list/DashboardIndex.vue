...
const dateRange = ref([]); // Final confirmed range
...
// Allow all dates (or block future dates)
const allowedDates = (date) => {
  return true; // Change to: moment(date).isSameOrBefore(moment(), 'day') to block future
};

// 🧠 Handle manual range selection
const handleManualDateRange = (date) => {
  if (!rangeStart.value || (rangeStart.value && rangeEnd.value)) {
    ...
    fetchLeads();
    menu.value = false; // Close the calendar
  }
};
...
const fetchLeads = async () => {
  ...
  try {
    ...
    if (dateRange.value.length > 0) {
      params.set('start_date', moment(dateRange.value[0]).format('YYYY-MM-DD'));
      params.set('end_date', moment(dateRange.value[1]).format('YYYY-MM-DD') ?? moment(dateRange.value[0]).format('YYYY-MM-DD'));
    } else {
