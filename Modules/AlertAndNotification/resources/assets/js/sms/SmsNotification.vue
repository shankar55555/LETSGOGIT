...
<template>
  <IconBtn id="notification-btn" v-if="$can('smsLog', 'view')">
    <VTooltip bottom>
      <template v-slot:activator="{ on, attrs }">
        <VBadge v-bind="on" color="error" :content="unreadCount" offset-x="2" offset-y="3">
          <VIcon size="24" icon="tabler-message-dots" />
        </VBadge>
      </template>
    ...
    <VMenu ref="notificationMenu" activator="parent" min-width="300px" location="bottom end" offset="8">
      <VCard class="d-flex flex-column">
        ...
        <!-- Footer -->
        <VCardText v-show="(notification_list.length || unreadCount > 0) && $can('smsLog', 'view')" class="pa-4">
...
const getNotificationCount = async () => {
  try {
    const response = await $api('/sms/notification-count');
...
const getLatestFiveNotificationList = async () => {
  try {
    loading.value = true;
    const response = await $api('/sms/latest-five-notification-list');
...
const markAllReadOrUnread = async () => {
  try {
    ...
    const response = await $api('/sms/mark-all-read-or-un-read', { method: 'POST', body: { is_read: is_read } });
...
const isReadNotification = async (notification_id, is_read) => {
  try {
    const response = await $api('/sms/is-read-notification', { method: 'POST', body: { notification_id: notification_id, is_read: is_read } });
    toast.success(response.message);
    await getNotificationCount(),
      await getLatestFiveNotificationList();
  } catch (error) {
...
onMounted(async () => {
  ...
  echo.channel(`notification-sms-channel-${userId}`).listen(".new-notification", (data) => {
    console.log('notification-sms-channel ', JSON.stringify(data));
