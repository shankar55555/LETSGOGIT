<template>
  <section v-if="$can('smsLog', 'view')">
        ...
        <div class="d-flex justify-space-between flex-wrap gap-y-4">
          ...
          <div class="d-flex flex-row gap-4 align-center flex-wrap">
            ...
            <VSelect v-model="searchStatus" @update:modelValue="(value) => getLogList()" label="Select Status"
              ...
              item-title="status_text" item-value="slug" />
      ...
      <VCardText v-else class="px-0">
        <VDataTableServer v-model:items-per-page="pagination.per_page" :items="LogList" :items-length="LogList.length"
          ...
          <template #item.status="{ item }">
            <template v-if="editingStatusId === item.id && $can('smsLog', 'edit')">
          ...
          <template #item.actions="{ item }">
            <VIcon v-if="$can('smsLog', 'delete')" icon="tabler-trash" color="primary" variant="elevated" :size="20"
    ...
    <DeleteDialog v-model:isDialogVisible="isDeleteDialogOpen" confirm-title="Delete!"
      ...
      :action="'force_delete'" :endpoint="`/sms/delete-notification/${logInfo?.id}`"
...
const tableHeaderSlug = ref('sms-log-header-list');
const headers = ref([]);
...
const getLogList = async () => {
  ...
  try {
    ...
    const response = await $api('/sms/log-list', { params });
...
const openContentDialog = item => {
  if (item.is_read_by_user == false) {
    $api('/sms/update-read-status', { method: "POST", body: { id: item.id } }).then(res => {
...
const handleItemClick = async (item) => {
  if (item.is_read_by_user == false) {
    await $api('/sms/update-read-status', { method: "POST", body: { id: item.id } }).then(res => {
      item.is_read_by_user = true
    }).catch(e => { })
  }
...
const updateStatusValue = async (item) => {
  try {
    const res = await $api(`/sms/status-update`, {
      method: 'POST',
...
onMounted(async () => { await fetchStatusList(NOTIFICATION_LOG); await getLogList(); });
</script>
