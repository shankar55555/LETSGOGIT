<template>
  <section v-if="$can('leads', 'view')">
    <VCard class="mb-6">
      <VCardText>
        <div class="d-flex justify-end">
          <div class="d-flex gap-3">
            ...
            <VBtn v-if="$can('reachout', 'send-message')" class="mr-3" @click="reachoutSendMessage">Send Message
            </VBtn>
      ...
      <div class="d-flex gap-2 justify-start" v-if="showSearchFilter || showStatusFilter">
        ...
        <div class="d-flex flex-row gap-4 align-center flex-wrap">
          <VSelect label="Lead Status" class="ml-5" v-model="selectedStatus" :items="statusList"
      ...
      <VCardText v-else class="px-0">
        <VDataTableServer v-model:items-per-page="pagination.per_page" :items="leadList" :items-length="leadList.length"
          ...
          <template #item.status="{ item }">
            <VChip :color="item.status_info ? item.status_info.status_color : 'primary'" class="font-weight-medium"
              size="small">
              {{ item.status_info ? item.status_info.status_text : item.status }}
            </VChip>
          </template>
    ...
    <WhatsAppSendMessage v-if="isDialogVisible" :currentInfo="currentInfo" :selectedIdList="selectedIdList"
      @submit="clearSearchFilter" v-model:isDialogVisible="isDialogVisible" :type="'Lead'" />
  </section>
</template>
...
const tableHeaderSlug = ref('reachout-lead-header-list');
const headers = ref([]);
...
const getList = async () => {
  ...
  try {
    ...
    const response = await $api('/leads', { params });
    leadList.value = response.data ?? [];
    pagination.value = { current_page: response.meta.current_page, last_page: response.meta.last_page, total: response.meta.total, per_page: response.meta.per_page, from: response.meta.from, to: response.meta.to };
  } catch (error) {
