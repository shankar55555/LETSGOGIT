<template>
  <section v-if="$can('bellNotification', 'view')">
    ...
    <VRow>
      ...
      <VCol cols="12" md="5" lg="4">
        <VCard>
          <VCardTitle>
            <h3 class="mb-2">Bell Notification Utilities</h3>
          </VCardTitle>
          <VCardText>
      ...
      <VCol cols="12" md="7" lg="8">
        <VCard>
          ...
          <VCardText>
            <VForm ref="formRef" v-model="formValid">
              <VRow class="mt-5">
                ...
                <VCol cols="12">
                  <VLabel>Bell Notification Message <span style="color: red;">*</span></VLabel>
                  <VTextarea v-model="templateInfo.bell_notification_message" :rules="[requiredValidator]"
                    placeholder="Enter Message" rows="2" outlined dense auto-grow />
                </VCol>
              </VRow>
            </VForm>
          </VCardText>
          ...
          <VCardActions class="mt-10 mb-3">
            <VSpacer />
            <VBtn v-if="$can('bellNotification', 'preview')" variant="elevated" @click="preview"
              :loading="loadingPreview" :disabled="loadingPreview">
              Preview
            </VBtn>
            <VBtn v-if="$can('bellNotification', 'edit')" variant="elevated" @click="saveData" :loading="loadingSave"
    ...
    <VDialog v-model="preview_dialog" max-width="900px" scrollable persistent>
      <VCard>
        <VCardTitle class="d-flex flex-wrap gap-4">
          <h3 class="page-title">Preview Bell Notification</h3>
        </VCardTitle>
        <VDivider />
        ...
        <VCardActions class="mt-10 mb-3">
          ...
          <VBtn v-if="$can('bellNotification', 'send-message')" variant="elevated" @click="checkSendMessage"
            :loading="sendLoading" :disabled="sendLoading">Send Bell Notification</VBtn>
        </VCardActions>
      </VCard>
    </VDialog>
    <!-- 👉 Delete Dialog -->
...
const bread = ref([
  ...
  { title: "Bell Notification Utility", disabled: true, to: "" },
])
...
const setVariableContent = (template) => {
  ...
  templateInfo.value.bell_notification_message = `${templateInfo.value.bell_notification_message || ""} ${set_name.replace("name", variable)}`
}
...
const clickOnItem = (item) => {
  ...
  template.bell_notification_message = (template.bell_notification_message || '').replace(/<br\s*\/?>/gi, '').replace(/amp;/g, '');
...
const getCategoryList = async () => {
  ...
  try {
    const response = await $api("/bell/category-list");
    ...
    if (firstType) {
      ...
      templateInfo.value.bell_notification_message = (templateInfo.value.bell_notification_message || '').replace(/<br\s*\/?>/gi, '').replace(/amp;/g, '');
...
const preview = async () => {
  ...
  try {
    const { data } = await $api("/bell/preview", {
      method: "POST",
      body: { notification_type_id: templateInfo.value.notification_type_id },
    });
    const message = (data?.bell_notification_message || '').replace(/<br\s*\/?>/gi, '').replace(/amp;/g, '');
    previewMailData.value = data?.bell_notification_message || message;
...
const saveData = async () => {
  ...
  try {
    const obj = {
      ...
      bell_notification_message: templateInfo.value.bell_notification_message,
    ...
    const response = await $api(`/bell/create-update-template/${templateInfo.value.id}`, { method: "PUT", body: obj });
...
const checkSendMessage = async () => {
  ...
  try {
    const response = await $api("/bell/send-notification", {
      method: "POST",
      body: { notification_type_id: templateInfo.value.notification_type_id },
    });
