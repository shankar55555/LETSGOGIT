...
<template>
  <IconBtn id="notification-btn" v-if="$can('bellNotificationLog', 'view')">
    <VTooltip bottom>
    ...
    <VMenu ref="notificationMenu" activator="parent" min-width="300px" location="bottom end" offset="8">
      <VCard class="d-flex flex-column notification_card_w">
        <!-- Header -->
                ...
                <div class="d-flex align-start gap-3 w-100">
                  ...
                  <div class="flex-grow-1">
                    ...
                    <VListItemTitle class="text-sm font-weight-medium">
                      {{ changedSubject(notification.subject) }}
                    </VListItemTitle>
        ...
        <!-- Footer -->
        <VCardText v-show="(notification_list.length || unreadCount > 0) && $can('bellNotificationLog', 'view')"
          class="pa-4">
...
const getNotificationCount = async () => {
  try {
    const response = await $api('/bell/notification-count');
...
const changedContent = (subject) => {
  // Remove <br> and 'amp;' as before
  let clean = subject.replace(/<br\s*\/?\>/gi, '').replace(/amp;/g, '');
  // Split into words
  const words = clean.split(/\s+/);
  if (words.length > 7) {
    const sliced = words.slice(0, 7).join(' ') + '...';
    console.log('Sliced content:', sliced);
    return sliced;
  } else {
    return clean;
  }
...
const changedSubject = (subject) => {
  let clean = subject.replace(/<br\s*\/?\>/gi, '').replace(/amp;/g, '');
  const words = clean.split(/\s+/);
  if (words.length > 7) {
    const sliced = words.slice(0, 7).join(' ') + '...';
    console.log('Sliced subject:', sliced);
    return sliced;
  } else {
    return clean;
  }
...
const getLatestFiveNotificationList = async () => {
  try {
    loading.value = true;
    const response = await $api('/bell/latest-five-notification-list');
...
const markAllReadOrUnread = async () => {
  try {
    ...
    const response = await $api('/bell/mark-all-read-or-un-read', { method: 'POST', body: { is_read: is_read } });
...
const isReadNotification = async (notification_id, is_read) => {
  try {
    const response = await $api('/bell/is-read-notification', { method: 'POST', body: { notification_id: notification_id, is_read: is_read } });
    toast.success(response.message);
    await getNotificationCount(),
      await getLatestFiveNotificationList();
  } catch (error) {
...
onMounted(async () => {
  ...
  echo.channel(`notification-bell-channel-${userId}`).listen(".new-notification", (data) => {
    console.log('notification-bell-channel ', JSON.stringify(data));
