<template>
  <section v-if="$can('bellNotificationLog', 'view')">
      ...
      <VCardText v-else class="px-0">
        <VDataTableServer v-model:items-per-page="pagination.per_page" :items="LogList" :items-length="LogList.length"
          ...
          <template #item.status="{ item }">
            <template v-if="editingStatusId === item.id && $can('bellNotificationLog', 'edit')">
          ...
          <template #item.actions="{ item }">
            <VIcon v-if="$can('bellNotificationLog', 'delete')" icon="tabler-trash" color="primary" variant="elevated"
    ...
    <DeleteDialog v-model:isDialogVisible="isDeleteDialogOpen" confirm-title="Delete!"
      ...
      :action="'force_delete'" :endpoint="`/bell/delete-notification/${logInfo?.id}`"
...
const tableHeaderSlug = ref('bell-notification-log-header-list');
const headers = ref([]);
...
const getLogList = async () => {
  ...
  try {
    ...
    const response = await $api('/bell/log-list', { params });
...
const openContentDialog = item => {
  if (item.is_read_by_user == false) {
    $api('/bell/update-read-status', { method: "POST", body: { id: item.id } }).then(res => {
...
const handleItemClick = async (item) => {
  if (item.is_read_by_user == false) {
    await $api('/bell/update-read-status', { method: "POST", body: { id: item.id } }).then(res => {
      item.is_read_by_user = true
    }).catch(e => { })
  }
...
const updateStatusValue = async (item) => {
  try {
    const res = await $api(`/bell/status-update`, {
      method: 'POST',
