    ...
    <VTooltip bottom>
      <template v-slot:activator="{ on, attrs }">
        <VBadge v-bind="on" color="error" :content="unreadCount" offset-x="2" offset-y="3">
          <VIcon size="24" icon="tabler-brand-whatsapp" />
        </VBadge>
      </template>
...
const getNotificationCount = async () => {
  try {
    const response = await $api('/whatsApp/notification-count');
...
const getLatestFiveNotificationList = async () => {
  try {
    loading.value = true;
    const response = await $api('/whatsApp/latest-five-notification-list');
...
const markAllReadOrUnread = async () => {
  try {
    ...
    const response = await $api('/whatsApp/mark-all-read-or-un-read', { method: 'POST', body: { is_read: is_read } });
...
const isReadNotification = async (notification_id, is_read) => {
  try {
    const response = await $api('/whatsApp/is-read-notification', { method: 'POST', body: { notification_id: notification_id, is_read: is_read } });
    toast.success(response.message);
    await getNotificationCount(),
      await getLatestFiveNotificationList();
  } catch (error) {
...
onMounted(async () => {
  ...
  echo.channel(`notification-whats-app-channel-${userId}`).listen(".new-notification", (data) => {
    console.log('notification-whats-app-channel ', JSON.stringify(data));
