<template>
  <section v-if="$can('whatsApp', 'view')">
    ...
    <VRow>
      ...
      <VCol cols="12" md="5" lg="4">
        <VCard>
          <VCardTitle>
            <h3 class="mb-2">Whats App Utilities</h3>
          </VCardTitle>
          <VCardText>
      ...
      <VCol cols="12" md="7" lg="8">
        <VCard>
            ...
            <VForm ref="formRef" v-model="formValid">
              <VRow class="mt-5">
                ...
                <!-- <VCol cols="12">
                  ...
                  <VTextField v-model="templateInfo.email_subject" class="mb-4" placeholder="Subject of Email" outlined
                    dense readOnly hide-details />
                </VCol> -->
                ...
                <VCol cols="12">
                  <VLabel>Whats App Message <span style="color: red;">*</span></VLabel>
                  <VTextarea v-model="templateInfo.whats_app_message" :rules="[requiredValidator]"
          ...
          <VCardActions class="mt-10 mb-3">
            <VSpacer />
            <VBtn v-if="$can('whatsApp', 'preview')" variant="elevated" @click="preview" :loading="loadingPreview"
              :disabled="loadingPreview">
              Preview
            </VBtn>
            <VBtn v-if="$can('whatsApp', 'edit')" variant="elevated" @click="saveData" :loading="loadingSave"
    ...
    <VDialog v-model="preview_dialog" max-width="900px" scrollable persistent>
      <VCard>
        ...
        <VCardActions class="mt-10 mb-3">
          ...
          <VBtn v-if="$can('whatsApp', 'send-message')" variant="elevated" @click="checkSendMessage"
            :loading="sendLoading" :disabled="sendLoading">Send Whats App Message</VBtn>
        </VCardActions>
      </VCard>
    </VDialog>
    <!-- 👉 Delete Dialog -->
...
const bread = ref([
  ...
  { title: "Whats App Utility", disabled: true, to: "" },
])
...
const setVariableContent = (template) => {
  ...
  templateInfo.value.whats_app_message = `${templateInfo.value.whats_app_message || ""} ${set_name.replace("name", variable)}`
}
...
const clickOnItem = (item) => {
  ...
  template.whats_app_message = (template.whats_app_message || '').replace(/<br\s*\/?>/gi, '').replace(/amp;/g, '');
...
const getCategoryList = async () => {
  ...
  try {
    const response = await $api("/whatsApp/category-list");
    ...
    if (firstType) {
      ...
      templateInfo.value.whats_app_message = (templateInfo.value.whats_app_message || '').replace(/<br\s*\/?>/gi, '').replace(/amp;/g, '');
...
const preview = async () => {
  ...
  try {
    const { data } = await $api("/whatsApp/preview", {
      method: "POST",
      body: { notification_type_id: templateInfo.value.notification_type_id },
    });

    const message = (data?.whats_app_message || '').replace(/<br\s*\/?>/gi, '').replace(/amp;/g, '');
    previewMailData.value = data?.whats_app_message || message;
...
const saveData = async () => {
  ...
  try {
    const obj = {
      ...
      whats_app_message: templateInfo.value.whats_app_message,
      notification_type_id: templateInfo.value.notification_type_id,
      type_title: itemTypes.value,
      description: itemDescription.value,
    };

    const response = await $api(`/whatsApp/create-update-template/${templateInfo.value.id}`, { method: "PUT", body: obj });
...
const checkSendMessage = async () => {
  ...
  try {
    const response = await $api("/whatsApp/send-notification", {
      method: "POST",
      body: { notification_type_id: templateInfo.value.notification_type_id },
    });
