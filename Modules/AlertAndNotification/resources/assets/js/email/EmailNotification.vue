  ...
  <IconBtn id="notification-btn">
    ...
    <VMenu ref="notificationMenu" activator="parent" min-width="300px" location="bottom end" offset="8">
      <VCard class="d-flex flex-column">
            ...
            <template v-for="(notification, index) in notification_list" :key="notification.id">
              ...
              <VListItem link lines="one" min-height="60" class="list-item-hover-class"
                ...
                <div class="d-flex align-start gap-3">
                  <VAvatar size="40" :image="notification.sender?.avatar || undefined"
                    :icon="notification.icon || undefined" :variant="notification.sender?.avatar ? undefined : 'tonal'">
                    <span v-if="notification.sender">{{ avatarText(notification.sender.name) }}</span>
                  </VAvatar>
                  <div>
                    <VListItemTitle class="text-sm font-weight-medium">{{ notification.subject }}</VListItemTitle>
        ...
        <!-- Footer -->
        <VCardText v-show="(notification_list.length || unreadCount > 0) && $can('bellNotification', 'view')"
          class="pa-4">
...
const getNotificationCount = async () => {
  try {
    const response = await $api('/email/notification-count');
...
const getLatestFiveNotificationList = async () => {
  try {
    loading.value = true;
    const response = await $api('/email/latest-five-notification-list');
...
const markAllReadOrUnread = async () => {
  try {
    ...
    const response = await $api('/email/mark-all-read-or-un-read', { method: 'POST', body: { is_read: is_read } });
...
const isReadNotification = async (notification_id, is_read) => {
  try {
    const response = await $api('/email/is-read-notification', { method: 'POST', body: { notification_id: notification_id, is_read: is_read } });
    toast.success(response.message);
    await getNotificationCount(),
      await getLatestFiveNotificationList();
  } catch (error) {
...
onMounted(async () => {
  ...
  echo.channel(`notification-email-channel-${userId}`).listen(".new-notification", (data) => {
    console.log('notification-email-channel ', JSON.stringify(data));
