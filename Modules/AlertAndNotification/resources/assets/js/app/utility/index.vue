<template>
  <section v-if="$can('appUtility', 'view')">
    ...
    <VRow>
      ...
      <VCol cols="12" md="5" lg="4">
        <VCard>
          <VCardTitle>
            <h3 class="mb-2">App Utilities</h3>
          </VCardTitle>
          <VCardText>
      ...
      <VCol cols="12" md="7" lg="8">
        <VCard>
          ...
          <VCardText>
            <VForm ref="formRef" v-model="formValid">
              <VRow class="mt-5">
                ...
                <VCol cols="12">
                  <VLabel>App Message <span style="color: red;">*</span></VLabel>
                  <VTextarea v-model="templateInfo.app_message" :rules="[requiredValidator]" placeholder="Enter Message"
          ...
          <VCardActions class="mt-10 mb-3">
            <VSpacer />
            <VBtn v-if="$can('appUtility', 'preview')" variant="elevated" @click="preview" :loading="loadingPreview"
              :disabled="loadingPreview">
              Preview
            </VBtn>
            <VBtn v-if="$can('appUtility', 'edit')" variant="elevated" @click="saveData" :loading="loadingSave"
    ...
    <VDialog v-model="preview_dialog" max-width="900px" scrollable persistent>
      <VCard>
        <VCardTitle class="d-flex flex-wrap gap-4">
          <h3 class="page-title">Preview App Notification</h3>
        </VCardTitle>
        <VDivider />
        ...
        <VCardActions class="mt-10 mb-3">
          ...
          <VBtn v-if="$can('appUtility', 'send-message')" variant="elevated" @click="checkSendMessage"
            :loading="sendLoading" :disabled="sendLoading">Send App Message</VBtn>
        </VCardActions>
      </VCard>
    </VDialog>
...
const bread = ref([
  ...
  { title: "App Utility", disabled: true, to: "" },
])
...
const setVariableContent = (template) => {
  ...
  templateInfo.value.app_message = `${templateInfo.value.app_message || ""} ${set_name.replace("name", variable)}`
}
...
const clickOnItem = (item) => {
  ...
  const template = item.app_message || {};
  template.app_message = (template.app_message || '').replace(/<br\s*\/?>/gi, '').replace(/amp;/g, '');
...
const getCategoryList = async () => {
  ...
  try {
    const response = await $api("/app/category-list");
    ...
    if (firstType) {
      ...
      templateInfo.value.app_message = (templateInfo.value.app_message || '').replace(/<br\s*\/?>/gi, '').replace(/amp;/g, '');
...
const preview = async () => {
  ...
  try {
    const { data } = await $api("/app/preview", {
      method: "POST",
...
const saveData = async () => {
  ...
  try {
    const obj = {
      ...
      app_message: templateInfo.value.app_message,
      notification_type_id: templateInfo.value.notification_type_id,
      type_title: itemTypes.value,
      description: itemDescription.value,
    };

    const response = await $api(`/app/create-update-template/${templateInfo.value.id}`, { method: "PUT", body: obj });
...
const checkSendMessage = async () => {
  ...
  try {
    const response = await $api("/app/send-notification", {
      method: "POST",
      body: { notification_type_id: templateInfo.value.notification_type_id },
    });
