<template>
  <section v-if="$can('appLog', 'view')">
      ...
      <VCardText v-else class="px-0">
        <VDataTableServer v-model:items-per-page="pagination.per_page" :items="LogList" :items-length="LogList.length"
          ...
          <template #item.status="{ item }">
            <template v-if="editingStatusId === item.id && $can('appLog', 'edit')">
          ...
          <template #item.actions="{ item }">
            <VIcon v-if="$can('appLog', 'delete')" icon="tabler-trash" color="primary" variant="elevated" :size="20"
    ...
    <DeleteDialog v-model:isDialogVisible="isDeleteDialogOpen" confirm-title="Delete!"
      ...
      :action="'force_delete'" :endpoint="`/app/delete-notification/${logInfo?.id}`"
...
const tableHeaderSlug = ref('app-log-header-list');
const headers = ref([]);
...
const getLogList = async () => {
  ...
  try {
    ...
    const response = await $api('/app/log-list', { params });
...
const openContentDialog = item => {
  if (item.is_read_by_user == false) {
    $api('/app/update-read-status', { method: "POST", body: { id: item.id } }).then(res => {
...
const handleItemClick = async (item) => {
  if (item.is_read_by_user == false) {
    await $api('/app/update-read-status', { method: "POST", body: { id: item.id } }).then(res => {
      item.is_read_by_user = true
    }).catch(e => { })
  }
...
const updateStatusValue = async (item) => {
  try {
    const res = await $api(`/app/status-update`, {
      method: 'POST',
