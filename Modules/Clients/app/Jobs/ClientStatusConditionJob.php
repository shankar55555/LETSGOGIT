...
class ClientStatusConditionJob implements ShouldQueue
{
    use Dispatchable, InteractsWithQueue, Queueable, SerializesModels;

    private const LOG_PREFIX = '[ClientStatusConditionJob]';
    ...
    public function handle(): void
    {
        ...
        try {
            $statuses = AdminControlConfig::where('status_for', CommonConst::MODULE_CLIENT)
            ...
            foreach ($statuses as $status) {
                $rules = $ruleHelper->commonStatusConditionRules(CommonConst::MODULE_CLIENT, $status->slug);
                ...
                $ids = Client::where('status', $status->slug)->pluck('id')->toArray();

                if (empty($ids)) {
                    i(self::LOG_PREFIX . " No Clients found with status: {$status->slug}");
                    continue;
                }

                foreach ($rules as $rule) {
                    $matchedIds = Client::getMatchingIdsFromRule($rule, $ids);

                    foreach ($matchedIds as $id) {
                        try {
                            $notificationHelper->handle(null, clientRuleNotification($id), $rule->id, loginUserId());
                            i(self::LOG_PREFIX . " Notification sent for Client ID: {$id}");
                        } catch (\Exception $e) {
                            er(self::LOG_PREFIX . " Notification failed for Client ID: {$id}. Error: {$e->getMessage()}");
                        }
                    }
                }
            }
